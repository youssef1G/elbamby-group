...
export default function Checkout() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const earnEstimate = earnRate > 0 ? Math.floor(Number(order.total ?? 0) * earnRate) : 0;
      ...
      if (err.code === 'STOCK_CONFLICT' && err.items?.length) {
        ...
        setError('root.serverError', { message: t('checkout:errors.redemptionDisabled') });
      } else if (err.code === 'INVALID_REDEMPTION') {
        setError('root.serverError', { message: t('checkout:errors.redemptionRequiresLogin') });
      } else {
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-xl mx-auto px-5 py-24 text-center">
        <h2 className="text-heading-lg text-bg-text-primary mb-3">{t('cart.empty')}</h2>
        <Link to="/shop" className="btn-primary text-sm">{t('nav.shop')}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 sm:px-8 py-10 sm:py-14">
      ...
      >
        <Link to="/cart" className="hover:text-bg-primary-500">{t('nav.cart')}</Link>
        <span>/</span>
      ...
      <div className="grid lg:grid-cols-5 gap-10">
        ...
        >
          ...
          <div className="flex items-start gap-3 bg-bg-primary-500/10 border border-bg-primary-500/30 rounded-2xl px-5 py-4">
            <Banknote size={20} strokeWidth={1.5} className="shrink-0 text-bg-primary-500 mt-0.5" />
            <div>
          ...
          {!customerAuthLoading && customer && (
            <div className="surface-card p-6 mb-6">
              ...
              <p className="text-xs text-bg-text-secondary mb-4">
                {t('checkout:redeem.balance', { points: balance.toLocaleString('en-US') })}
              </p>
              ...
              <p className="text-[11px] text-bg-text-secondary mt-2">
                {redeemRate != null
                  ? t('checkout:redeem.rateHint', { rate: formatPrice(redeemRate) })
            ...
            <div className="border-t border-bg-border pt-4 space-y-2">
              ...
              {pointsDiscountEgp > 0 && (
                <div className="flex justify-between text-xs text-bg-success">
                  ...
                  <span className="ltr-nums">−{formatPrice(pointsDiscountEgp)}</span>
                </div>
              )}
